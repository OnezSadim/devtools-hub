...
export default function VelocityCalculator() {
  ...
  const calculate = () => {
    ...
    if (mode === "velocity") setResult((a / b).toFixed(4) + " m/s");
    else if (mode === "distance") setResult((a * b).toFixed(4) + " m");
    else setResult((a / b).toFixed(4) + " s");
  };
  const modes = [
    { id: "velocity", label: "Velocity", l1: "Distance (m)", l2: "Time (s)" },
    { id: "distance", label: "Distance", l1: "Velocity (m/s)", l2: "Time (s)" },
    { id: "time", label: "Time", l1: "Distance (m)", l2: "Velocity (m/s)" },
  ];
  const cur = modes.find(m => m.id === mode)!;
  return (
    <main className="min-h-screen bg-gray-950 text-white p-8">
      <div className="max-w-lg mx-auto">
        <h1 className="text-3xl font-bold mb-2">Velocity Calculator</h1>
        <p className="text-gray-400 mb-6">Calculate velocity, distance, or time (v = d/t)</p>
