  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-8">
      <div className="max-w-xl mx-auto">
        ...
        <p className="text-gray-400 mb-6">Calculate light refraction between two media.</p>
        <div className="space-y-4 bg-gray-900 p-6 rounded-xl">
          <div><label className="block text-sm text-gray-400 mb-1">Index of Refraction n₁</label>
            <input className="w-full bg-gray-800 rounded px-3 py-2 text-white" value={n1} onChange={e => setN1(e.target.value)} placeholder="e.g. 1.0 (air)" /></div>
          <div><label className="block text-sm text-gray-400 mb-1">Index of Refraction n₂</label>
            <input className="w-full bg-gray-800 rounded px-3 py-2 text-white" value={n2} onChange={e => setN2(e.target.value)} placeholder="e.g. 1.5 (glass)" /></div>
          <div><label className="block text-sm text-gray-400 mb-1">Incident Angle θ₁ (degrees)</label>
            <input className="w-full bg-gray-800 rounded px-3 py-2 text-white" value={angle1} onChange={e => setAngle1(e.target.value)} placeholder="e.g. 30" /></div>
